...
def main():
    ...
    #AQUI hay que declarar una lista que almacene la perdida
    perdidas = []
    numi=0
    
    for epoch in range(args.num_train_epochs):
        print("       Iniciando entrenamiento con extraccion de datos...")
        unet.train()
        if args.train_text_encoder:
            text_encoder.train()
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                if args.with_prior_preservation:
                    ...
                    #declarar variable que redondea valor y lo almacena (varPerdida)
                    varPerdida = loss.detach().item()
                    #print()
                    #print("Valor Actual de perdida",varPerdida)
                    perdidas.append(varPerdida)
                else:
            ...
            fll=round(fll/4)
            #print()
            #print("Decada de entrenamiento actual: ",fll)
            pr=bar(fll)
            
            logs = {"loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "Decada":fll}
